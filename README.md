...
* **Processing Layer (Algorithm):**
    * **One Euro Filter:** Minimizes jitter while maintaining high responsiveness for micro-movements.
...
Simply open `index.html` in a modern browser (Chrome/Edge).

> **Note:** Ensure the backend is running *first* to establish the socket connection.
...
直接在现代浏览器（Chrome/Edge）中打开 `index.html` 文件。

> **注意：** 请确保后端服务器已启动，否则无法建立 WebSocket 连接。
